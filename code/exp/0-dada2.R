...
# DADA2 analysis
# Sequencing: Illumina mi-seq November 2023
...
# Plot quality profile of forward reads
plotQualityProfile(fnFs[9])
# Plot quality profile of reverse reads
plotQualityProfile(fnRs[9])
...
# identify taxonomy
taxa <- assignTaxonomy(seqtab.nochim, "data/SILVA138.1/silva_nr99_v138.1_train_set.fa.gz", multithread=8, tryRC=TRUE)

# exact species matching (won't work if concatenating sequences)
taxa <- addSpecies(taxa,  "data/SILVA138.1/silva_species_assignment_v138.1.fa.gz", allowMultiple = TRUE, tryRC = TRUE)

#Inspect the taxonomic assignments
taxa.print <- taxa # Removing sequence rownames for display only
rownames(taxa.print) <- NULL
head(taxa.print)
capture.output(taxa.print, file ="data/exp/bioinfo/taxa_dada2.txt")

# Clean and save environment ----------------------------------------------

#Save environment
save.image("data/exp/bioinfo/0-dada2_complete.RData")
...
# correct error in sample names 157-pl-c
rownames(seqtab.nochim)[which(rownames(seqtab.nochim) == "157-pl-c")] <- "157-ctrl-e"
sample.names[which(sample.names == "157-pl-c")] <- "157-ctrl-e"

# correct error in sample 105
rownames(seqtab.nochim)[which(rownames(seqtab.nochim) == "105-NANA-lp")] <- "105-pl-l"
sample.names[which(sample.names == "105-NANA-lp")] <- "105-pl-l"

save.image("data/exp/bioinfo/0-dada2.RData")
